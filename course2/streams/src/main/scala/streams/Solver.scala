...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filterNot { case(block, history) => explored(block) }
    /*for {
  ...
  def from(initial: LazyList[(Block, List[Move])], explored: Set[Block]): LazyList[(Block, List[Move])] = {
    if(initial.isEmpty) LazyList.empty
      ...
      val nextSteps = for {
        ...
        validNeighbors <- newNeighborsOnly(neighbors, explored)
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    val moves = List.empty
    val paths = LazyList((startBlock, moves))
    from(paths, Set.empty)
  }
  ...
  lazy val solution: List[Move] =
    if(pathsToGoal.isEmpty) Nil
      ...
      val (block, validPath) = pathsToGoal.head
      validPath.reverse
    }
}
