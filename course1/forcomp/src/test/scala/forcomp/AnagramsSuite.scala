...
class AnagramsSuite {
  ...
  @Test def `wordOccurrences: dbabcdabcd (3pts)`: Unit =
    assertEquals(List(('a', 2), ('b', 3), ('c', 2), ('d', 3)), wordOccurrences("dbabcdabcd"))
  ...
  @Test def `sentenceOccurrences: abcd e abcd f (5pts)`: Unit =
    assertEquals(List(('a', 2), ('b', 2), ('c', 2), ('d', 2), ('e', 1), ('f', 1)), sentenceOccurrences(List("abcd", "e", "abcd", "f")))
  ...
  @Test def `subtract: larder - ra (10pts)`: Unit = {
    val lard = List(('a', 1), ('d', 1), ('l', 1), ('r', 2))
    val ra = List(('a', 1), ('r', 1))
    val lrd = List(('d', 1), ('l', 1), ('r', 1))
    assertEquals(lrd, subtract(lard, ra))
  }

   @Test def `subtract: larder - [Nil] (10pts)`: Unit = {
    val lard = List(('a', 1), ('d', 1), ('l', 1), ('r', 2))
    val empty = List()
    assertEquals(lard, subtract(lard, empty))
  }
