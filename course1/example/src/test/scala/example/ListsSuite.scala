 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of negative numbers numbers (10pts)`: Unit = {
    Assert.assertEquals(-3, sum(List(-1,-2,0)))
  }

  @Test def `sum of a list of repeated numbers (10pts)`: Unit = {
    Assert.assertEquals(3, sum(List(1,1,1)))
  }
  ...
  @Test def `sum of a positive and negative numbers (10pts)`: Unit = {
    Assert.assertEquals(0, sum(List(-1,-2,1,2)))
  }



  @Test def `max of positive and negative numbers (10pts)`: Unit = {
    Assert.assertEquals(3, max(List(3, -7, 2)))
  }

  @Test def `max of empty list should throw java.util.NoSuchElementException (10pts)`: Unit = {
     try {
       max(List())
       Assert.fail("No exception has been thrown")
     } catch {
  ...
  @Test def `max of repeated numbers (10pts)`: Unit = {
    Assert.assertEquals(99, max(List(99,1,99,1,99)))
  }
